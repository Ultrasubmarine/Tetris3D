       ...
       private void Start()
        {
            ...
            _uiBoxAnimation 
                .Append(_evilBoxRectTransform.DOAnchorPosY(Screen.height / 2 + 100, _time / 2).From(Vector2.up * (Screen.height / 2 - 300)))
                .Join(boxCanvas.DOFade(1f, _time / 2.5f).From(0f))
                .Append(_evilBoxRectTransform.DOAnchorPosY(Screen.height / 2 , _timeMoving)
                    .From(Vector2.up *(Screen.height / 2 + 100)).SetLoops(3, LoopType.Yoyo))
