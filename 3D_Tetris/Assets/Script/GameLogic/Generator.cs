...
public class Generator : MonoBehaviour {
    ...
    //    Destroy(exElement);
        ...
        exElement.transform.position = new Vector3( temp.x, plane.gameObject.transform.position.y + minYforElement, temp.z);
    ...
    private List<Vector3> PovFreeCount( Vector3 point, bool[,,] matrix, ElementScript currEl)
    {
        ...
        if (point.x < 2)
            if (matrix[(int)point.x + 1, (int)point.y, (int)point.z] && (point.y == 0 || !matrix[(int)point.x + 1, (int)point.y - 1, (int)point.z]))
            {
        ...
        if (point.z < 2)
            if (matrix[(int)point.x, (int)point.y, (int)point.z + 1] && (point.y == 0 || !matrix[(int)point.x, (int)point.y - 1, (int)point.z + 1]))
            {
